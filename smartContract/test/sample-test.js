...
describe("WorkToken", function () {
  it("deploy", async function () {
    ...
    const testERC20 = await TestERC20.deploy(ethers.BigNumber.from("0xfffffffffffffffffffffffff"));
    await testERC20.deployed();

    await workToken.setTokenAddresses(testERC20.address);

    const tx0 = await testERC20.transfer(addr2.address, ethers.BigNumber.from("0xffffffffffffffffffff"))
    await tx0.wait();
    ...
    await testERC20.approve(workToken.address, ethers.BigNumber.from("0xffffffffffffffffffff"))

    const tx4 = await workToken.mintNFT(addr1.address);
    await tx4.wait();
